...
run("v.state='practice';nextItem()");for(let i=0;i<6;i++)run('selected=v.item.answer;submit();nextItem()');assert.equal(run('v.state'),'ready');run('startSession()');sample();const before=run('v.trials.length');els['a'+run('v.item.answer')].onclick();assert.equal(run('v.trials.length'),before);assert.equal(run('v.state'),'task');els.submit.onclick();assert.equal(run('v.points'),1);run('takeBreak()');sample();assert.equal(run('v.heart_rate.at(-1).phase'),'break');assert.equal(els.hrvalue,undefined);
