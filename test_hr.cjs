...
run("v.state='practice';nextItem()");for(let i=0;i<6;i++)run('selected=v.item.answer;submit();nextItem()');assert.equal(run('v.state'),'ready');run('startSession()');sample();run('selected=v.item.answer;submit()');assert.equal(run('v.points'),1);run('takeBreak()');sample();assert.equal(run('v.heart_rate.at(-1).phase'),'break');assert.equal(els.hrvalue,undefined);
...
run('startSession()');assert.equal(run('current().ratioIndex'),0);run("finish('voluntary_end')");const count=run('v.heart_rate.length');sample();assert.equal(run('v.heart_rate.length'),count);assert.equal(run('hr.connected'),false);run('results()');assert.ok(els.hrcsv&&els.rrcsv);
